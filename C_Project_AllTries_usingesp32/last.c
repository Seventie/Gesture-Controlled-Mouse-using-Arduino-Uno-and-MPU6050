...
int main() {
    ...
    fprintf(file, "Timestamp,SensorData,GestureLabel\n");
    ...
    for (int gesture = 0; gesture < GESTURE_COUNT; gesture++) {
        ...
        while (GetTickCount() - startTime < RECORD_DURATION) {
            ...
            if (ReadFile(serial_fd, buffer, sizeof(buffer) - 1, &bytesRead, NULL)) {
                ...
                if (bytesRead > 0) {
                    // Write data to CSV file with the gesture label
                    if (fprintf(file, "%lu,%s,%d\n", GetTickCount(), buffer, gesture + 1) < 0) {
                        fprintf(stderr, "Failed to write to CSV.\n");
                    }
                    fflush(file);  // Ensure data is written to file immediately
                    printf("Data written to CSV: %s | Gesture Label: %d\n", buffer, gesture + 1);  // Print data to console for verification
                } else {
