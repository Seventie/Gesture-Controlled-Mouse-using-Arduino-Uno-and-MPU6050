...
// Function to validate if a string contains only numeric values and commas
int is_valid_sensor_data(const char *data) {
    while (*data) {
        if (!isdigit(*data) && *data != ',' && *data != '.' && *data != '-') {
...
int main() {
    ...
    FILE *file = fopen("gesture_data_1_1.csv", "w");
    if (file == NULL) {
    ...
    while (GetTickCount() - startTime < RECORD_DURATION) {
        ...
        if (ReadFile(serial_fd, buffer, sizeof(buffer) - 1, &bytesRead, NULL)) {
            ...
            if (bytesRead > 0) {
                // Check if the data is valid sensor data before writing to CSV
                if (is_valid_sensor_data(buffer)) {
                    if (fprintf(file, "%s\n", buffer) < 0) {
                    ...
                    printf("Invalid data received: %s\n", buffer);  // Ignore and log invalid data
                }
            } else {
